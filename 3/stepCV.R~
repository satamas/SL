...
stepCV <- function (object, scope, scale = 0, direction = c("both", "backward", 
    ...
    k = 2, ...) 
{
    extractCV <- function(x, scale, k, ...) UseMethod("extractCV")

    extractCV.lm <- function(x, scale, k, ...){
        t <- tune(lm, x$call$formula, data = eval(x$call$data))
        c(t$best.model$rank, t$best.performance)
    }

    extractCV.glm <- function(x, scale, k, ...){
        t <- tune(glm, x$call$formula, data = eval(x$call$data))
        c(t$best.model$rank, t$best.performance)
    }
